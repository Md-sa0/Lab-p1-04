...
try:
    ...
    print("Erro: O módulo 'torch' (PyTorch) não está instalado neste ambiente.")
    print("Por favor, instale-o executando no terminal: pip install torch")
    sys.exit(1)
...
if __name__ == "__main__":
    ...
    print("--- Simulação: Transformer Completo ---")
    print(f"Entrada no Encoder: {encoder_input_text}")
    print("Iniciando loop auto-regressivo no Decoder...\n")
    ...
    with torch.no_grad():
        for i in range(max_length):
            ...
            palavras_decoder_atual = [idx_to_vocab[idx] for idx in decoder_input_ids]
            print(f"Iteração {i+1}: Token previsto '{target_word}' -> Nova entrada: {palavras_decoder_atual}")
            
            if next_word_id == vocab["<EOS>"]:
                print("\nToken <EOS> alcançado. Geração interrompida.")
                break
                
    final_translation = [idx_to_vocab[idx] for idx in decoder_input_ids if idx not in (vocab["<START>"], vocab["<EOS>"])]
    print(f"\nResultado Final: {' '.join(final_translation)}")